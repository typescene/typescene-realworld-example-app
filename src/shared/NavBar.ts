import { Application, bind, service, UICell, UICloseColumn, UICloseRow, UIHeading1, UIImage, UILinkButton, UIOppositeRow, UIRow, ViewComponent } from "typescene";
...
const LogoHeading = UIHeading1.with({
    ...
    onClick() {
        this.getCompositeParent(Application)!.navigate("/");
    }
});
...
export default class NavBar extends ViewComponent.with(UICell.with(
    {
    ...
    goToProfile() {
        this.getParentComponent(Application)!
            .navigate("/profile/" + this.user!.profile!.username)
    }
}
