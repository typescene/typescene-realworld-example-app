...
export default class NavBar extends ViewComponent.with(
    UICell.with(
        ...
        UICell.with(
            ...
            UIRow.with(LogoHeading, UICloseColumn.with({ dimensions: { grow: 1 } }, LoggedOutLinks, LoggedInLinks))
        )
    )
) {
    @service("App.User")
    ...
    goToProfile() {
        this.getParentComponent(Application)!.navigate("/profile/" + this.user!.profile!.username);
    }
}
